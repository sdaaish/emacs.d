...
#+BEGIN_SRC emacs-lisp
  (use-package elfeed
    ...
    :config
    (setq elfeed-db-directory (concat my-emacs-dir "/shared/elfeeddb"))
...
#+BEGIN_SRC emacs-lisp
  (use-package elfeed-org
    :init (elfeed-org)
    :config (setq rmh-elfeed-org-files (list (concat org-directory "/elfeed.org"))))
#+END_SRC
**** Elfeed-goodies
Some extras
#+BEGIN_SRC emacs-lisp
...
A ~hydra~ for elfeed.
#+BEGIN_SRC emacs-lisp
(defhydra my/hydra-elfeed (:hint nil)
   "
   _d_efault   _c_ode   _e_macs   _S_ecurity   _t_oday   _w_eek   _*_star
   _b_rowse   Mark as _R_ead   _M_ark  _L_ogs
   _Q_uit elfeed   _q_uit    _p_: previous line   _n_: next line
   "
   ("b" (elfeed-search-browse-url))
   ("d" (elfeed-search-set-filter "@6-months-ago"))
   ("c" (elfeed-search-set-filter "@6-months-ago +code"))
   ("e" (elfeed-search-set-filter "@6-months-ago +emacs"))
   ("S" (elfeed-search-set-filter "@6-months-ago +security"))
   ("t" (elfeed-search-set-filter "@1-days-ago +unread"))
   ("w" (elfeed-search-set-filter "@7-days-ago +unread"))
...
#+BEGIN_SRC emacs-lisp
  ...
  _c_heat.sh   _E_ww         _f_ree-keys _H_ugo localhost
  ...
  "
    ...
    ("h" man "man")
    ("H" (browse-url "http://localhost:1313/"))    
    ("p" (powershell))
...
#+END_SRC
*** Refile
Define targets for refile. Store them in a file, one target per line.
#+BEGIN_SRC emacs-lisp
  ...
  (setq my/refile-targets (read-lines (concat org-directory ".target-files")))

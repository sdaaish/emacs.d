...
#+SUBTITLE: A personal emacs configuration
...
#+end_src
*** Server
Start emacs as server. This don't work under the current setup with *Emacs 28.1* in windows.
#+begin_src emacs-lisp
(server-start)
(setq server-log t)
#+end_src
...
#+end_src
*** Certificates
Add some extra CA's for Emacs that are missing on Windows et al.
[[https://letsencrypt.org/certificates/][Let's Encrypt]] as an example.
...
#+BEGIN_SRC emacs-lisp
(defhydra hydra-zoom (global-map "<f2> z" :hint nil :timeout 3)
  "
    Change text-scale.
    Press _g_ to zoom in, _l_ out.
    _q_ to quit.
    "
...
#+BEGIN_SRC emacs-lisp
  (defhydra hydra-font (global-map "C-<f2>" :timeout 3)
...
(use-package flycheck
  ...
  :hook ((emacs-lisp-mode powershell-mode sh-mode) . (lambda () (flycheck-mode 1))))
#+end_src
*** Avy
#+begin_src emacs-lisp
...
More files
#+begin_src emacs-lisp
(setq lispdir (let (lispdir) (cond ((eq system-type 'windows-nt)
        (expand-file-name ".config/lisp/" (getenv "USERPROFILE")))
...
Change font for the =echo= area (messages from Emacs) and also for the =minibuffer=, input text.
From Emacs Wiki.
#+begin_src emacs-lisp
(with-current-buffer (get-buffer " *Echo Area 0*")
  (setq-local face-remapping-alist '((default (:height 1.2) fixed-pitch))))

(custom-set-faces
 '(minibuffer-prompt ((t (:foreground "#51afef" :height 1.2)))))
...
#+END_SRC
*** Language
Spellchecking, use Flycheck with aspell.
Need to load some other dictionaries. And to fix the *flycheck* setup.
Read more at [[https://github.com/leotaku/flycheck-aspell/blob/master/README-FLYCHECK.md][Flycheck-aspell]].
#+BEGIN_SRC emacs-lisp
...
(use-package denote
  ...
  (denote-dired-directories (expand-file-name "notes/denote" my/onedrive-dir))
  ...
  :bind
  ("C-c n a" . my/denote-create-new-note-from-region)
  ...
  ("C-c n m" . #'denote-link-add-missing-links)
...
#+END_SRC
** Local name calendar
Celebrate names in calendar, load in =org-agenda=.
From https://github.com/matsl/sv-kalender-namnsdagar/.
#+begin_src emacs-lisp
...
From [[https://irreal.org/blog/?p=69][Irreal]].
#+begin_src emacs-lisp
...
(use-package powershell
  ...
  :commands powershell
  :config
  (setq powershell-eldoc-def-files (list (expand-file-name "etc/eldoc/powershell-eldoc.el" my/init-dir)))
  (if (eq system-type 'gnu/linux)

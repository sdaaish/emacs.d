...
+ Use-package is uxseful to load packages from archives more easily. But you need to load that before it is used, and you must ensure that it is installed before you call it.
** Package-archives
Load packages
...
#+BEGIN_SRC emacs-lisp
(setq dropbox-dir (concat (getenv "HOME") "/Dropbox/org/"))
